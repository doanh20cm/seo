...
namespace CoccoCore\CPT\Shortcodes\ProductListSimple;
...
class ProductListSimple implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Product List - Simple', 'cocco' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'Sale', 'cocco' )         => 'sale',
								esc_html__( 'Best Sellers', 'cocco' ) => 'best-sellers',
								esc_html__( 'Featured', 'cocco' )     => 'featured'
							),
							'save_always' => true
						),
						array(
							...
							'description' => esc_html__( 'Number of products to show (default value is 4)', 'cocco' )
						),
                        array(
                            'type'        => 'dropdown',
	...
	public function render( $atts, $content = null ) {
		...
		$params['holder_column_classes'] = $this->getHolderColumnClasses($params, $default_atts);
		...
		$html = cocco_mikado_get_woo_shortcode_module_template_part( 'templates/product-list-template', 'product-list-simple', '', $params );
	...
	private function getHolderColumnClasses($params, $default_atts){
        $holderColumnClasses = array();

        $holderColumnClasses[] = ! empty( $params['space_between_items'] ) ? 'mkdf-' . $params['space_between_items'] . '-space' : 'mkdf-' . $default_atts['space_between_items'] . '-space';
        $holderColumnClasses[] = $this->getColumnNumberClass( $params );

        return implode(" ", $holderColumnClasses);
    }
