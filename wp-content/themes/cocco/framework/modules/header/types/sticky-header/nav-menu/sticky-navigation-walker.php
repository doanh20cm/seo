...
if (!class_exists('CoccoMikadoStickyNavigationWalker')) {
	class CoccoMikadoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide") {
					...
					$wide_type = cocco_mikado_options()->getOptionValue('wide_dropdown_style');
					if ( $wide_type !== '') {
						$menu_type .= " mkdf-wide-menu-".$wide_type;
					}
				} else {
					$menu_type = " narrow";
				}
