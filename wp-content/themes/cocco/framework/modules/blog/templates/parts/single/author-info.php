<?php
$author_info_box       = esc_attr( cocco_mikado_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( cocco_mikado_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = cocco_mikado_core_plugin_installed() ? cocco_mikado_get_user_custom_fields() : false;
$display_author_social = cocco_mikado_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
			...
			<div class="mkdf-author-description-content">
				<div class="mkdf-author-description-image">
					<a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>" target="_self">
						<?php echo cocco_mikado_kses_img( get_avatar( get_the_author_meta( 'ID' ), 201) ); ?>
					</a>
				</div>
						...
						<?php if ( is_array( $social_networks ) && count( $social_networks ) ) { ?>
							<div class="mkdf-author-social-icons clearfix">
								<?php foreach ( $social_networks as $network ) { ?>
									<a itemprop="url" href="<?php echo esc_url( $network['link'] ) ?>" class="<?php echo esc_attr($network['class']); ?>" target="_blank"></a>
								<?php } ?>
							</div>
						<?php } ?>
					<?php } ?>
