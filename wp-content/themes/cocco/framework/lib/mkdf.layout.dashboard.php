...
/*
   Class: CoccoMikadoDashboardForm
   A class that initializes Mikado Dashboard Container
*/
class CoccoMikadoDashboardForm implements iCoccoMikadoLayoutNode, iCoccoMikadoRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render($factory) {
		...
		<form method="<?php echo esc_attr($this->form_method);?>" id="<?php echo esc_attr($this->form_id);?>" class="<?php echo esc_attr($action_class)?>" data-action="<?php echo esc_attr($this->form_action);?>">
			...
			if ( cocco_mikado_core_plugin_installed() ) {
				echo cocco_mikado_get_button_html( array(
...
/*
   Class: CoccoMikadoDashboardGroup
   A class that initializes CoccoMikado Group Field
*/
class CoccoMikadoDashboardGroup implements iCoccoMikadoLayoutNode, iCoccoMikadoRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: CoccoMikadoDashboardTitle
   A class that initializes Dashboard Title
*/
class CoccoMikadoDashboardTitle implements iCoccoMikadoRender {
	private $name;
	private $title;
...
/*
   Class: CoccoMikadoDashboardField
   A class that initializes CoccoMikado Front Field
*/

class CoccoMikadoDashboardField implements iCoccoMikadoRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class CoccoMikadoDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class CoccoMikadoDashboardFieldText extends CoccoMikadoDashboardFieldType {
...
class CoccoMikadoDashboardFieldTextArea extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class CoccoMikadoDashboardFieldImage extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="mkdf-dashboard-gallery-holder">
				<ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset($value_html) ) {
						echo cocco_mikado_get_module_part( $value_html );
					} ?>
				</ul>
				<div class="mkdf-dashboard-gallery-uploader">
					...
					if ( mkdf_membership_theme_installed() ) {
						echo cocco_mikado_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'cocco' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" target="_self" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'cocco' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e('Remove Media','cocco'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class CoccoMikadoDashboardFieldGallery extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class CoccoMikadoDashboardFieldSelect extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class CoccoMikadoDashboardFieldDate extends CoccoMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class CoccoMikadoDashboardFieldIcon extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = cocco_mikado_icon_collections()->getIconCollectionsEmpty();
...
class CoccoMikadoDashboardFieldColor extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class CoccoMikadoDashboardFieldCheckBoxGroup extends CoccoMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class CoccoMikadoDashboardFieldAddress extends CoccoMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
...
class CoccoMikadoDashboardRepeater implements iCoccoMikadoRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ($this->table_layout) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'cocco' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'cocco' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class CoccoMikadoDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new CoccoMikadoDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new CoccoMikadoDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new CoccoMikadoDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new CoccoMikadoDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new CoccoMikadoDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new CoccoMikadoDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new CoccoMikadoDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new CoccoMikadoDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new CoccoMikadoDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new CoccoMikadoDashboardFieldAddress();
