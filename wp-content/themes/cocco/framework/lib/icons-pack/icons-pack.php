...
include MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	include_once $module_load;
}

if ( ! function_exists( 'cocco_mikado_get_icon_pack_collections' ) ) {
	function cocco_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'cocco_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'cocco_mikado_get_icon_pack_collections' );
}

if ( ! function_exists( 'cocco_mikado_get_icon_pack_options' ) ) {
	function cocco_mikado_get_icon_pack_options() {
		$options = apply_filters( 'cocco_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'cocco_mikado_get_icon_pack_options' );
}

if ( ! function_exists( 'cocco_mikado_get_icon_pack_social_options' ) ) {
	function cocco_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'cocco_mikado_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'cocco_mikado_get_icon_pack_social_options' );
}

/*
 * Class: CoccoMikadoIconCollections
 */
class CoccoMikadoIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = cocco_mikado_get_icon_pack_collections( true );
		$this->iconPackOptions       = cocco_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = cocco_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = cocco_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'cocco' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'cocco' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'cocco' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'cocco' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'cocco' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'cocco' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'cocco' )       => 'fa-lg',
			esc_html__( 'Small', 'cocco' )      => 'fa-2x',
			esc_html__( 'Medium', 'cocco' )     => 'fa-3x',
			esc_html__( 'Large', 'cocco' )      => 'fa-4x',
			esc_html__( 'Very Large', 'cocco' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iCoccoMikadoIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'cocco' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'cocco' ) ] = '';
