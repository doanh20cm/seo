...
if ( ! function_exists( 'cocco_mikado_add_linear_icon_pack' ) ) {
	function cocco_mikado_add_linear_icon_pack( $iconCollections ) {
		$iconCollections['linear_icons'] = new CoccoMikadoLinearIcons( 'Linear Icons', 'linear_icon' );
		
		return $iconCollections;
	}
	
	add_filter( 'cocco_mikado_filter_add_icon_pack_into_collection', 'cocco_mikado_add_linear_icon_pack' );
}

if ( ! function_exists( 'cocco_mikado_add_linear_icon_pack_option' ) ) {
	function cocco_mikado_add_linear_icon_pack_option( $options ) {
		$options['linear_icons'] = esc_html__( 'Linear Icons', 'cocco' );
		
		return $options;
	}
	
	add_filter( 'cocco_mikado_filter_add_icon_pack_into_options', 'cocco_mikado_add_linear_icon_pack_option' );
}

class CoccoMikadoLinearIcons implements iCoccoMikadoIconCollection {
	public $icons;
	public $title;
	public $param;
	public $styleUrl;
	...
	function __construct($title_label = "", $param = "") {
		...
		$this->param = $param;
		$this->setIconsArray();
		$this->styleUrl = MIKADO_FRAMEWORK_ICONS_ROOT . '/linear-icons/style.css';
	}
	...
	public function render($icon, $params = array()) {
		...
		$html .= '<span aria-hidden="true" class="mkdf-icon-linear-icons lnr ' . $icon . ' ' . $iconClass . '" ' . $iconAttributesString . '></span>';
