...
if (!function_exists('cocco_mikado_add_font_awesome_icon_pack')) {
    function cocco_mikado_add_font_awesome_icon_pack($iconCollections)
    {
        $iconCollections['font_awesome'] = new CoccoMikadoIconsFontAwesome('Font Awesome', 'fa_icon');

        return $iconCollections;
    }

    add_filter('cocco_mikado_filter_add_icon_pack_into_collection', 'cocco_mikado_add_font_awesome_icon_pack');
}

if (!function_exists('cocco_mikado_add_font_awesome_icon_pack_option')) {
    function cocco_mikado_add_font_awesome_icon_pack_option($options)
    {
        $options['font_awesome'] = esc_html__('Font Awesome', 'cocco');

        return $options;
    }

    add_filter('cocco_mikado_filter_add_icon_pack_into_options', 'cocco_mikado_add_font_awesome_icon_pack_option');
    add_filter('cocco_mikado_filter_add_icon_pack_into_social_options', 'cocco_mikado_add_font_awesome_icon_pack_option');
}

class CoccoMikadoIconsFontAwesome implements iCoccoMikadoIconCollection
{
    public $icons;
    public $title;
    public $param;
    ...
    function __construct($title_label = "", $param = "")
    {
        ...
        $this->styleUrl = MIKADO_FRAMEWORK_ICONS_ROOT . '/font-awesome/css/fontawesome-all.min.css';
    }
    ...
    private function getNewIconsArray($brandsIcons = false)
    {
        ...
        $iconsArray = array();
        $icons = json_decode($wp_filesystem->get_contents(MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/font-awesome/icons.json'));
