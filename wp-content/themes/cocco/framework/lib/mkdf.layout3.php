...
/*
   Class: CoccoMikadoMultipleImages
   A class that initializes Mikado Multiple Images
*/

class CoccoMikadoMultipleImages implements iCoccoMikadoRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$cocco_mikado_Framework->mkdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="mkdf-gallery-uploader">
								<a class="mkdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'cocco' ); ?></a>
								<a class="mkdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'cocco' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class CoccoMikadoTwitterFramework implements iCoccoMikadoRender {
	public function render( $factory ) {
		$twitterApi = CoccoTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'cocco' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'cocco' ) : esc_html__( 'Connect with Twitter', 'cocco' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'cocco' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'cocco' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class CoccoMikadoInstagramFramework implements iCoccoMikadoRender {
	public function render( $factory ) {
		$instagram_api = CoccoInstagramApi::getInstance();
		$message       = '';
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'cocco' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'cocco' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'cocco' );
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'cocco' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'cocco' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ( $instagram_user_id ) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'cocco' );
					...
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'cocco' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'cocco' ); ?></a>
							</div>
							<!--<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php /*echo esc_url( $instagram_api->facebookRequestCode() ); */?>"><?php /*echo esc_html__( 'Connect with Instagram Business account', 'cocco' ); */?></a>
							</div>-->
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'cocco' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class CoccoMikadoRepeater implements iCoccoMikadoRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'cocco' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="mkdf-repeater-table-heading">
					<div class="mkdf-repeater-fields-holder">
						<div class="mkdf-repeater-table-cell mkdf-repeater-sort"><?php esc_html_e( 'Order', 'cocco' ) ?></div>
						...
						<div class="mkdf-repeater-table-cell mkdf-repeater-remove"><?php esc_html_e( 'Remove', 'cocco' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="mkdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="mkdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = cocco_mikado_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class CoccoMikadoFieldAddress extends CoccoMikadoFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'cocco' ); ?></button>
							<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'cocco' ); ?></a>
						</div>
					</div>
				</div>
			</div>
...
class CoccoMikadoFieldIcon extends CoccoMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = cocco_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = cocco_mikado_icon_collections()->getIconCollectionsKeys();
		?>
            ...
            <div class="mkdf-section-content">
                <div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = cocco_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						$style       = 'display: none';
						if ( !empty($rvalue) && $rvalue['icon_pack'] == $icons_collection ) {
							$style = 'display: block';
						}
						?>
                        <div class="row mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                            <div class="col-lg-<?php echo esc_attr($col_width); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" class="<?php echo esc_attr($select2) ?> form-control mkdf-form-element">
									<?php
									$icons      = cocco_mikado_icon_collections()->getIconCollection( $icons_collection );
