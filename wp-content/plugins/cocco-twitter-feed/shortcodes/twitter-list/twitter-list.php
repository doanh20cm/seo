...
namespace CoccoTwitter\Shortcodes\TwitterList;

use CoccoTwitter\Lib;
...
class TwitterList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Twitter List', 'cocco-twitter-feed' ),
					'base'                      => $this->base,
					'category'                  => esc_html__( 'by COCCO', 'cocco-twitter-feed' ),
					...
					'params'                    => array(
						array(
							'type'        => 'textfield',
							'param_name'  => 'user_id',
							'heading'     => esc_html__( 'User ID', 'cocco-twitter-feed' ),
							'admin_label' => true
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'number_of_columns',
							'heading'     => esc_html__( 'Number of Columns', 'cocco-twitter-feed' ),
							'value'       => array(
								esc_html__( 'One', 'cocco-twitter-feed' )   => '1',
								esc_html__( 'Two', 'cocco-twitter-feed' )   => '2',
								esc_html__( 'Three', 'cocco-twitter-feed' ) => '3',
								esc_html__( 'Four', 'cocco-twitter-feed' )  => '4',
								esc_html__( 'Five', 'cocco-twitter-feed' )  => '5'
							),
							'save_always' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Space Between Columns', 'cocco-twitter-feed' ),
							'value'      => array_flip( cocco_mikado_get_space_between_items_array() )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'number_of_tweets',
							'heading'    => esc_html__( 'Number of Tweets', 'cocco-twitter-feed' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'transient_time',
							'heading'    => esc_html__( 'Tweets Cache Time', 'cocco-twitter-feed' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$twitter_api           = new \CoccoTwitterApi();
		...
		$html = cocco_twitter_get_shortcode_module_template_part( 'holder', 'twitter-list', '', $params );
