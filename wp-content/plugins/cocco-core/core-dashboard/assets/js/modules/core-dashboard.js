(function ($) {
	...
	/**
	 *  All functions to be called on $(document).ready() should be in mkdfImport function test
	 **/
	...
	var mkdfImport = {
		...
		importAttachments: function (repeat) {
			...
			if (!repeat) {
				for (var i = 1; i < 20; i++) {
					var xml = i < 20 ? 'cocco_content_' + i + '.xml' : 'cocco_content_' + i + '.xml';
					var data = {
						action: 'content',
						xml: xml,
						images: mkdfImport.importImages
