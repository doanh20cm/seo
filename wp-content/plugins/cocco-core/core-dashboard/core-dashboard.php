...
if(!class_exists('MikadoCoreDashboard')){
	...
	class MikadoCoreDashboard {
		...
		public	$licence_field = 'cocco_purchase_info';
		public	$import_field = 'cocco_import_params';
		...
		function __construct() {
			...
			add_action( 'mkdf_mikado_action_core_on_activate', array(&$this, 'set_redirect') );
		...
		function dashboard_add_page() {
			...
			if (cocco_core_theme_installed()) {
				...
				$page = add_menu_page(
					esc_html__('Cocco Dashboard', 'mkdf-core'),
					esc_html__('Cocco Dashboard', 'mkdf-core'),
					'administrator',
					'mkdf_core_dashboard',
					array(&$this, 'mkdf_core_dashboard_template'),
					COCCO_CORE_URL_PATH . '/core-dashboard/assets/img/admin-logo-icon.png',
					3
				);
		...
		function mkdf_core_dashboard_template() {
			...
			echo mkdf_core_get_module_template_part('core-dashboard', 'core-dashboard', '', $params);
		}
		...
		function enqueue_styles(){
			wp_enqueue_style( 'mkdf-core-dashboard-style', plugins_url( COCCO_CORE_REL_PATH . '/core-dashboard/assets/css/core-dashboard.min.css' ));
		}

		function enqueue_scripts(){
			wp_enqueue_script( 'select2', MIKADO_FRAMEWORK_ROOT . '/admin/assets/js/select2.min.js', array(), false, true );
			wp_enqueue_script( 'mkdf-core-dashboard-script', plugins_url( COCCO_CORE_REL_PATH . '/core-dashboard/assets/js/modules/core-dashboard.js' ), array(), false, true );
			$global_variables = apply_filters( 'mkdf_core_dashboard_filter_js_global_variables', array() );

			wp_localize_script( 'mkdf-core-dashboard-script', 'mkdfCoreDashboardGlobalVars', array(
		...
		public function register_sub_pages() {
			...
			$sub_pages = apply_filters( 'mkdf_core_filter_add_sub_page', $icons = array() );
		...
		function set_redirect() {
			if ( ! is_network_admin() ) {
				set_transient( '_mkdf_core_welcome_page_redirect', 1, 30 );
			}
		}

		function page_welcome_redirect() {
			$redirect = get_transient( '_mkdf_core_welcome_page_redirect' );
			delete_transient( '_mkdf_core_welcome_page_redirect' );
			if ( $redirect ) {
		...
		function purchase_code_registration(){
			...
			if ( empty( $_POST ) || ! isset( $_POST ) ) {
				return esc_html__( 'All fields are empty', 'mkdf-core' );
			} else {
		...
		function register_purchase_code() {
			...
			if ( empty( $data['purchase_code'] ) || empty( $data['email'] ) ) {
				cocco_core_ajax_status( 'error', esc_html__( 'Purchase Code and Email are empty', 'mkdf-core' ), array(
					'purchase_code' => false,
					'email'         => false
				) );
			} elseif ( empty( $data['purchase_code'] ) ) {
				cocco_core_ajax_status( 'error', esc_html__( 'Purchase Code is empty', 'mkdf-core' ), array( 'purchase_code' => false ) );
			} elseif ( empty( $data['email'] ) ) {
				cocco_core_ajax_status( 'error', esc_html__( 'Email is empty', 'mkdf-core' ), array( 'email' => false ) );
			}
			...
			if ( isset( $json['success'] ) && $json['success'] ) {
				...
				cocco_core_ajax_status( 'success', $this->response_codes( $json['response_code'] ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] && ( isset( $json['data']['error'] ) && $json['data']['error'] == 404 ) ) {

				cocco_core_ajax_status( 'error', $this->response_codes( $json['response_code'] ), array( 'purchase_code' => false ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] && ( isset( $json['data']['error'] ) && $json['data']['error'] == 'used' ) ) {

				cocco_core_ajax_status( 'error', $this->response_codes( $json['response_code'],  $json['data']), array( 'already_used' => true ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] ) {

				cocco_core_ajax_status( 'error', $this->response_codes( $json['response_code'] ) );
			}
		}
		...
		function api_connection($url){
			...
			if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
				return new WP_Error( 'bad_request', esc_html__('Bad request', 'mkdf-core' ));
			}

			$json = json_decode( wp_remote_retrieve_body( $response ), true );

			if ( empty( $json ) || ! is_array( $json ) ) {
				return new WP_Error( 'invalid_response',  esc_html__('Invalid Response', 'mkdf-core' ) );
			}
		...
		function response_codes( $code, $data = array() ) {
			...
			switch ( $code ):
				...
				case 200:
					$message = esc_html__( 'Failed to validate code due to an error', 'mkdf-core' );
					break;
				case 400:
					$message = esc_html__( 'Parameter or argument in the request was invalid', 'mkdf-core' );
					break;
				case 401:
					$message = esc_html__( 'The authorization header is missing. Verify that your code is correct.', 'mkdf-core' );
					break;
				case 403:
					$message = esc_html__( 'Personal token is incorrect or does not have the required permission(s)', 'mkdf-core' );
					break;
				case 404:
					$message = esc_html__( 'The purchase code is invalid', 'mkdf-core' );
					break;
				case 601:
					$message = esc_html__( 'You successfully activated theme', 'mkdf-core' );
					break;
				case 602:
					$message = esc_html__( 'Code is valid', 'mkdf-core' );
					break;
				case 603:
					$message = esc_html__( 'You successfully added demo', 'mkdf-core' );
					break;
				case 604:
					$message = esc_html__( 'You successfully deregister theme', 'mkdf-core' );
					break;
				case 650:
                    $registered_url = '';
                    ...
                    $message = sprintf(
                        esc_html__( 'This code was already used to register another domain%s. Please deregister your code there so that you can use it for registering here.', 'mkdf-core' ),
                        $registered_url
                    );
					break;
				case 651:
					$message = esc_html__( 'Error occurred during activation', 'mkdf-core' );
					break;
				case 652:
					$message = esc_html__( 'Code is invalid', 'mkdf-core' );
					break;
				case 653:
					$message = esc_html__( 'Error occurred during adding', 'mkdf-core' );
					break;
				case 654:
					$message = esc_html__( 'Error occurred during deactivation', 'mkdf-core' );
					break;
			endswitch;
        ...
        function theme_validation() {
            $is_theme_active = cocco_core_theme_installed();
            cocco_core_ajax_status( 'success', '', array( 'is_theme_active' => $is_theme_active ) );
        }
        ...
        public function is_theme_registered() {
            $code = $this->get_code();
            return cocco_core_theme_installed() && $code;
        }
	}
