<?php
namespace CoccoCore\CPT\Shortcodes\CallToAction;
...
class CallToAction implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Call To Action', 'cocco-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Set Content In Grid', 'cocco-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Elements Proportion', 'cocco-core' ),
							'value'      => array(
								esc_html__( '80/20', 'cocco-core' ) => '80',
								esc_html__( '75/25', 'cocco-core' ) => '75',
								esc_html__( '66/33', 'cocco-core' ) => '66',
								esc_html__( '50/50', 'cocco-core' ) => '50'
							),
							'dependency' => array( 'element' => 'layout', 'value' => array( 'normal' ) )
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Button Top Margin (px)', 'cocco-core' ),
						...
						array(
							'type'       => 'textarea_html',
							'param_name' => 'content',
							'heading'    => esc_html__( 'Content', 'cocco-core' ),
							'value'      => esc_html__( 'I am test text for Call to Action shortcode content', 'cocco-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = cocco_core_get_shortcode_module_template_part( 'templates/call-to-action', 'call-to-action', '', $params );
	...
	private function getButtonHolderStyles( $params ) {
		...
		if ( ! empty( $params['button_top_margin'] ) && $params['layout'] === 'simple' ) {
			$styles[] = 'margin-top: ' . cocco_mikado_filter_px( $params['button_top_margin'] ) . 'px';
		}
